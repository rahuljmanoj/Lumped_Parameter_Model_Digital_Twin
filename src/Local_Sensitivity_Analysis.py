...
if __name__ == '__main__':
    ...
    save_root = (
        r"C:\Workspace\Post_Doc_Works_NTNU\Projects\2_SWE_Velocity_LV_Filling_Pressure_Digital_Twin"
        r"\3_Codes\Python\Data_Results\Local_SA"
    )
    ...
    for sub_id in subject_list:
        ...
        # OAT preload & afterload sensitivity (elastance optional)
        sim.run_sensitivity_analysis(
            preload_params=('V_tot', 'C_sv', 'R_mv'),
            afterload_params=('R_sys', 'C_sa', 'Z_ao'),
            elastance_params=('E_min', 'E_max', 't_peak'),  # <-- include t_peak here
            steps=[-0.10, 0.10],
            fix_axes=True,
            include_hr=True,  # <-- HR included
            hr_steps=[-0.10, 0.10]  # <-- HR ±10%
        )
